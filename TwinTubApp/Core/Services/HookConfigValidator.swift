...
/// Validates that Claude Code settings.json has proper hooks configuration for TwinTub
struct HookConfigValidator {
    ...
    /// Required hook event types for TwinTub to function
    ...
    static var hookScriptPath: String {
        NSString(string: "~/.claude/hooks/twintub_hook_bridge.sh").expandingTildeInPath
    }
            ...
            for hook in hookList {
                if let type = hook["type"] as? String,
                   ...
                   let command = hook["command"] as? String {
                    ...
                    if command.contains("twintub_hook_bridge.sh") {
                        return true
                    }
                }
            }
    ...
    static func autoFix() -> Bool {
        ...
        if !result.hookScriptExists {
            NSLog("[TwinTub] Cannot auto-fix: hook script not found at \(result.hookScriptPath)")
            return false
        }
        ...
        guard let data = try? Data(contentsOf: settingsURL),
            ...
            NSLog("[TwinTub] Cannot auto-fix: failed to read settings.json")
            return false
        }

        // Build hooks configuration
        let hookCommand = "$HOME/.claude/hooks/twintub_hook_bridge.sh"
        ...
        guard let updatedData = try? JSONSerialization.data(withJSONObject: json, options: [.prettyPrinted, .sortedKeys]) else {
            NSLog("[TwinTub] Cannot auto-fix: failed to serialize JSON")
            return false
        }

        do {
            try updatedData.write(to: settingsURL)
            NSLog("[TwinTub] Auto-fixed settings.json: added hooks for \(result.missingHooks.joined(separator: ", "))")
            return true
        } catch {
            NSLog("[TwinTub] Cannot auto-fix: failed to write settings.json: \(error)")
            return false
        }
    }
}
