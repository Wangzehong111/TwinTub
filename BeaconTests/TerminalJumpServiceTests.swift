...
final class TerminalJumpServiceTests: XCTestCase {
    ...
    func testBeaconEventNewFieldsDecoding() throws {
        let json = """
        {
        ...
        let event = try JSONDecoder.beaconEventDecoder.decode(BeaconEvent.self, from: data)

        XCTAssertEqual(event.terminalWindowID, "99")
        XCTAssertEqual(event.terminalPaneID, "7")
    }

    func testBeaconEventNewFieldsOptionalDecoding() throws {
        let json = """
        {
            "event": "UserPromptSubmit",
            "session_id": "test-456"
