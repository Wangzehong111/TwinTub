...
kill_existing_instances() {
  ...
  # Collect running TwinTubApp instances from both derived data and dist bundles.
  while IFS= read -r pid; do
    ...
    # Include debugger parents that can respawn TwinTubApp immediately.
    local parent
  ...
  done < <(pgrep -f "/(DerivedData|dist)/.*/TwinTubApp" || true)

  # Explicitly collect debugserver processes attached to TwinTub debug builds.
  while IFS= read -r pid; do
    [ -z "$pid" ] && continue
    if [[ "$seen" != *" $pid "* ]]; then
      helper_pids+=("$pid")
  ...
  done < <(pgrep -f "debugserver.*TwinTub.*/Build/Products/Debug/TwinTubApp" || true)
  ...
  if [ "${#pids[@]}" -gt 0 ]; then
    echo "Stopping existing TwinTubApp instances: ${pids[*]}"
...
if [ -f "$ROOT_DIR/hooks/twintub_hook_bridge.sh" ]; then
  mkdir -p "$HOME/.claude/hooks"
  cp "$ROOT_DIR/hooks/twintub_hook_bridge.sh" "$HOME/.claude/hooks/twintub_hook_bridge.sh"
...
<dict>
  ...
  <string>com.twintub.local.dev</string>
