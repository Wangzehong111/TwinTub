...
final class NotificationCenterDelegate: NSObject, @unchecked Sendable, UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(
        ...
        if let sessionID = userInfo["session_id"] as? String {
            NSLog("[Beacon] Notification clicked for session: \(sessionID)")
...
final class UserNotificationCenterSender: @unchecked Sendable, NotificationSending, NotificationAuthorizationRequesting {
    ...
    private let queue = DispatchQueue(label: "beacon.notification.center", qos: .utility)
    ...
    func send(title: String, body: String, sound: String, sessionID: String?) {
        center.getNotificationSettings { [weak self] settings in
            ...
            NSLog("[Beacon] Notification authorization status: \(status.rawValue) (\(status.name))")
            switch status {
            case .authorized, .provisional, .ephemeral:
                NSLog("[Beacon] Posting via UNUserNotificationCenter")
                self.postViaUserNotifications(title: title, body: body, sound: sound, sessionID: sessionID)
            case .notDetermined:
                NSLog("[Beacon] Authorization not determined, requesting...")
                self.center.requestAuthorization(options: [.alert, .sound, .badge]) { granted, _ in
                    if granted {
                        NSLog("[Beacon] Authorization granted, posting via UNUserNotificationCenter")
                        self.postViaUserNotifications(title: title, body: body, sound: sound, sessionID: sessionID)
                    } else {
                        NSLog("[Beacon] Authorization denied, falling back to AppleScript")
            ...
            case .denied:
                ...
                self.fallback.send(title: title, body: body, sound: sound, sessionID: sessionID)
            @unknown default:
                NSLog("[Beacon] Unknown authorization status, falling back to AppleScript")
                self.fallback.send(title: title, body: body, sound: sound, sessionID: sessionID)
            }
        }
    }
    ...
    private func postViaUserNotifications(title: String, body: String, sound: String, sessionID: String?) {
        ...
        center.add(request) { [weak self, shouldRestorePolicy] error in
            ...
            NSLog("[Beacon] UNUserNotificationCenter failed: \(error), falling back to AppleScript")
            self.fallback.send(title: title, body: body, sound: sound, sessionID: sessionID)
        }
    }
}
...
/// Because the script executes inside Beacon's process, macOS attributes the
/// notification to Beacon and shows Beacon's app icon.
final class InProcessAppleScriptSender: NotificationSending {
    private let fallbackQueue = DispatchQueue(label: "beacon.notification.fallback", qos: .utility)
...
public final class NotificationService: NotificationDispatching {
    ...
    public func postWaiting(session: SessionModel, escalated: Bool) {
        let title = escalated ? "Beacon Escalation" : "Beacon Waiting"
    ...
    public func postCompleted(session: SessionModel) {
        sender.send(title: "Beacon Done", body: "\(session.projectName) completed", sound: "default", sessionID: session.id)
    }

    public func postTerminated(session: SessionModel, reason: SessionTerminationReason) {
        sender.send(
            title: "Beacon Session Ended",
