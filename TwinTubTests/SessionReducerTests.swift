...
final class SessionReducerTests: XCTestCase {
    func testAllEventMappings() {
        let now = Date()
        let base = TwinTubEvent(event: .userPromptSubmit, sessionID: "s1", cwd: "/tmp/a", prompt: "run")
        ...
        let waiting = TwinTubEvent(event: .permissionRequest, sessionID: "s1", toolName: "bash")
        ...
        let done = TwinTubEvent(event: .stop, sessionID: "s1")
        ...
        let remove = TwinTubEvent(event: .sessionEnd, sessionID: "s1")
    ...
    func testOutOfOrderWaitingThenUserPromptResumesProcessing() {
        let now = Date()
        let waiting = TwinTubEvent(event: .permissionRequest, sessionID: "s2", toolName: "git")
        ...
        let resume = TwinTubEvent(event: .userPromptSubmit, sessionID: "s2", prompt: "y")
    ...
    // MARK: - Token-based tests

    func testUsageSegmentsForTokens() {
        // 默认 200,000 tokens 作为 100%
        XCTAssertEqual(SessionModel.segmentsForTokens(0), 0)
        XCTAssertEqual(SessionModel.segmentsForTokens(1), 1)
        XCTAssertEqual(SessionModel.segmentsForTokens(100_000), 5)  // 50%
        XCTAssertEqual(SessionModel.segmentsForTokens(120_000), 6)  // 60%
        XCTAssertEqual(SessionModel.segmentsForTokens(160_000), 8)  // 80%
        XCTAssertEqual(SessionModel.segmentsForTokens(180_000), 9)  // 90%
        XCTAssertEqual(SessionModel.segmentsForTokens(200_000), 10) // 100%
    }

    func testUsageSegmentsForTokensWithDifferentMaxContext() {
        // 测试不同的 maxContextTokens
        XCTAssertEqual(SessionModel.segmentsForTokens(100_000, maxContextTokens: 200_000), 5)  // 50%
        XCTAssertEqual(SessionModel.segmentsForTokens(100_000, maxContextTokens: 128_000), 8)  // ~78%
        XCTAssertEqual(SessionModel.segmentsForTokens(64_000, maxContextTokens: 128_000), 5)   // 50%
    }

    func testMaxContextTokensForModel() {
        // Opus 和 Sonnet 都是 200K
        XCTAssertEqual(SessionModel.maxContextTokens(for: "claude-opus-4-20250514"), 200_000)
        XCTAssertEqual(SessionModel.maxContextTokens(for: "claude-sonnet-4-20250514"), 200_000)
        XCTAssertEqual(SessionModel.maxContextTokens(for: "claude-opus-4"), 200_000)
        XCTAssertEqual(SessionModel.maxContextTokens(for: "claude-sonnet-4"), 200_000)
        XCTAssertEqual(SessionModel.maxContextTokens(for: "opus-4"), 200_000)
        XCTAssertEqual(SessionModel.maxContextTokens(for: "sonnet-4"), 200_000)

        // Haiku 约 128K
        XCTAssertEqual(SessionModel.maxContextTokens(for: "claude-haiku-3.5"), 128_000)
        XCTAssertEqual(SessionModel.maxContextTokens(for: "claude-3-5-haiku"), 128_000)
        XCTAssertEqual(SessionModel.maxContextTokens(for: "haiku"), 128_000)

        // nil 或未知模型使用默认 200K
        XCTAssertEqual(SessionModel.maxContextTokens(for: nil), 200_000)
        XCTAssertEqual(SessionModel.maxContextTokens(for: "unknown-model"), 200_000)
    }

    func testPostToolUseWithUsageTokens() {
        let now = Date()
        let base = TwinTubEvent(event: .userPromptSubmit, sessionID: "s1", cwd: "/tmp/a", prompt: "run")
        let processing = SessionReducer.reduce(current: nil, event: base, now: now)
        ...
        // 发送带 usageTokens 的 PostToolUse
        let usage = TwinTubEvent(
            event: .postToolUse,
            sessionID: "s1",
            usageTokens: 100_000,  // 50% of 200K
        ...
        XCTAssertEqual(usageModel.usageTokens, 100_000)
        XCTAssertEqual(usageModel.maxContextTokens, 200_000)
        XCTAssertEqual(usageModel.model, "claude-opus-4-20250514")
    ...
    func testSourceFieldsPropagateAndOverride() {
        ...
        let keepSource = TwinTubEvent(event: .postToolUse, sessionID: "s3")
        ...
        let overrideSource = TwinTubEvent(
