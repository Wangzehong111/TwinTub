...
private final class EventBox: @unchecked Sendable {
    ...
    private var value: TwinTubEvent?

    func set(_ event: TwinTubEvent) {
        lock.lock()
        value = event
        lock.unlock()
    }

    func get() -> TwinTubEvent? {
        lock.lock()
        defer { lock.unlock() }
        return value
    }
...
final class HookBridgeTests: XCTestCase {
    ...
    func testHookBridgeMapsAndPostsPayload() async throws {
        ...
        process.environment = ["TWINTUB_PORT": String(port)]
    ...
    func testHookBridgeSilentFailureWhenAppOffline() throws {
        ...
        process.executableURL = URL(fileURLWithPath: "/bin/bash")
        process.arguments = [scriptPath]
        process.environment = ["TWINTUB_PORT": "59999"]
    ...
    private func bridgePath() throws -> String {
        ...
        let repoRoot = testFile
            ...
            .deletingLastPathComponent()
        let script = repoRoot.appendingPathComponent("hooks/twintub_hook_bridge.sh")
